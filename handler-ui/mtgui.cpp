...
void run_in_gui_thread(AnyQAppLambda* re){
    ...
    auto pApplication =qapplication();
    // thread safe!
    pApplication->postEvent(pApplication,new AnyQAppLambdaEvent(re));
...
void run_in_gui_thread_blocking(AnyQAppLambda* re){
    ...
    auto pApplication =qapplication();
    // thread safe!
    pApplication->postEvent(pApplication,new BlockingEvent(re,&done));
    while(!done){
